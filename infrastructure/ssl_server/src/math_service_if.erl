...
add(A,B,SslSocket,PacketNum)->
    {ok,CallBackModule}= application:get_env(?SERVICE, callBackModule),
    Msg={CallBackModule,add,[A,B],PacketNum},
    send(SslSocket,Msg).
    
divi(A,B,SslSocket,PacketNum)->
    {ok,CallBackModule}= application:get_env(?SERVICE, callBackModule),
    Msg={CallBackModule,divi,[A,B],PacketNum},
    send(SslSocket,Msg).
