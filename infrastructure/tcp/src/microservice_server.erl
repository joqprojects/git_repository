...
start_server(Port,ServerSetUp,CallbackModule,NumInstances)->
    gen_server:call(?MODULE, {start_server,Port,ServerSetUp,CallbackModule,NumInstances},infinity).
...
%% Returns: non
%% --------------------------------------------------------------------
handle_call({start_server,Port,ServerSetUp,CallbackModule,NumInstances}, _From,#state{pidsport=DbasePidsPort,portinfo=DbasePortInfo}=State) ->
  ...
  %  {ok,set}=misc_dets:set(?MODULE,Port,PortPool),
	...
	{error,no_entry}->
	    {ok,set}=misc_dets:set(DbasePortInfo,portInfo,[{Port,{LSock,ServerSetUp,CallbackModule,NumInstances}}]);
	 {ok,PortInfo}->
	    case lists:keyfind(Port,1,PortInfo) of
		{Port,_ServerInfo}->
		    NewPortInfo=lists:keyreplace(Port,1,PortInfo,{Port,{LSock,ServerSetUp,CallbackModule,NumInstances}});
		false ->
		    NewPortInfo=[{Port,{LSock,ServerSetUp,CallbackModule,NumInstances}}|PortInfo]
	    end,
...
handle_data(Socket,Bin,_CallbackModule)->
 %   io:format("~p~n",[{?MODULE,?LINE,CallbackModule}]),
    ...
    case {Type,Args} of
	{connect_req,[]}->
	    gen_tcp:send(Socket,term_to_binary(connect_ack));
	...
	{call,[CallbackModule,F,A]}->
	    ...
	    gen_tcp:send(Socket,term_to_binary({call_ack,Reply}));
	{cast,[CallbackModule,F,A]}->
	    rpc:cast(node(),erlang,apply,[CallbackModule,F,A]);
	X ->
	    gen_tcp:send(Socket,term_to_binary({call_ack,{error,unmatchedSignal,X}}))
    end.
